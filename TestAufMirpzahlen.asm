...
.text
	...
	beqz		$s5,	intervall		#R�ckgabe keine Primzahl -> n�chste Zahl
	
	jal			zahlUmdrehen			#Zahl umdrehen
	
	beq			$s2,	$s6,	intervall	#Test ob Palindrom -> wenn ja n�chste Zahl
	
	move		$s5,	$s2				#Umgredrehte Zahl auf Primzahl pr�fen
	jal			testPrim				
	...
	mul		$t4,	$t0,	4
	subi	$t4,	$t4,	4
	sw		$t3,	Feld($t4)			#Zahl wird rueckwaerts abgespeichert
	subi	$t0,	$t0,	1
	j		loop1
	...
	and		$t0,	$s5,	1			#Verknuepfen mit 1 (gerade/ungerade)
	...
	keinePrim:							#Wenn keine Primzahl dann Rueckgabewert 0
	li		$s5,	0
	jr		$ra
	
	istPrim:							#Wenn Primzahl dann Rueckgabewert 1
	li		$s5,	1
	jr		$ra